                    ...
                    <table class="table table-bordered mt-2" id='exportRm'>
                        ...
                        <tbody id="rawMaterialTable">
                            ...
                            <tr data-id="{{ $material->id }}">
                                ...
                                <td class="left-align rmname"><a href="{{ route('rawMaterial.edit', $material->id) }}" style="color: black;font-size:16px;text-decoration: none; text-align:left;">{{ $material->name }}</a></td> <!-- Raw Material Name -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="importModalLabel">Import for Raw Material</h5>
...
<script>
        let checkedRowsData = JSON.parse(localStorage.getItem('rawMaterialCheckedRows')) || [];
         let isEditing = false; // Track if edit mode is active
         let visibleData = [];
         let isFilter = false;

        // Function to update localStorage
        function rmupdateLocalStorage() {
        localStorage.setItem('rawMaterialCheckedRows', JSON.stringify(checkedRowsData));
        }

    // Function to create/update checked rows data
    function createCheckedRowsTable() {
    const rmtable = document.getElementById('rawMaterialTable');
    const rows = rmtable.querySelectorAll('tr');
    if(!isEditing){
    ...
    rows.forEach(row => {
        ...
        const rowData = {
            ...
            rmcode: row.querySelector('td:nth-child(4)')?.textContent.trim(),
...
// Event listener for checkbox changes
    ...
    const rmtable = document.getElementById('rawMaterialTable');
    rmtable.addEventListener('change', (event) => {
    ...
    document.addEventListener("DOMContentLoaded", function() {
           ...
           if (isFilter) {
                ...
                fetch(`/rawMaterials/export-all?statusValue=${encodeURIComponent(statusValue)}`)
                    ...
                    .then(data => {
                        ...
                        const wb = XLSX.utils.book_new();
                        XLSX.utils.book_append_sheet(wb, ws, 'RawMaterials');
                        XLSX.writeFile(wb, 'rawmaterials_all.xlsx');
                    })
                    .catch(error => {
            ...
            if (isFilter) {
                ...
                fetch(`/rawMaterials/export-all?statusValue=${encodeURIComponent(statusValue)}`)
                    ...
                    .then(data => {
                        ...
                        generatePdf(header, allData, 'rawmaterials_all.pdf');
                    })
                    .catch(error => {
        ...
        function generatePdf(header, data, filename) {
            ...
            doc.text("Raw Materials Export", 14, 15);
            doc.autoTable({
                head: [header],
                body: data,
                startY: 20,
        ...
        //     });
        // });
        document.getElementById('rawMaterialTable').addEventListener('change', function (e) {
